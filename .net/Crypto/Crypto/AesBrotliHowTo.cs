...
/// <summary>
///     Compress data using brotli and encrypt/decrypt data using Aes.
/// </summary>
/// <remarks>
...
/// </remarks>
public class AesBrotliHowTo
{
    /// <summary>
    ///     The supported custom version used in the custom header.
    /// </summary>
    private const byte SupportedVersion = 11;
    ...
    ///     In the next step the data is decompressed using brotli.
    /// </summary>
    ...
    )
    {
        ...
        var iv = await AesBrotliHowTo.ReadHeaderAsync(
            inputStream,
            cancellationToken);
        ...
        await brotliStream.CopyToAsync(
            outputStream,
            cancellationToken);
    }

    /// <summary>
    ///     Compresses the input data and encrypts the given <paramref name="data" /> using the <paramref name="key" /> and a
    ///     new <see cref="Aes.IV" />. The
    ...
    /// <summary>
    ///     Compresses the input data and encrypts the given <paramref name="inputStream" /> using the <paramref name="key" />
    ///     and a new
    ...
    )
    {
        ...
        await AesBrotliHowTo.WriteHeaderAsync(
            outputStream,
            aes.IV,
            cancellationToken);
    ...
    private static async Task<byte[]> ReadHeaderAsync(Stream stream, CancellationToken cancellationToken)
    {
        ...
        var actualLength = await stream.ReadAsync(
            version,
            cancellationToken);
        if (actualLength != 1 || version[0] != AesBrotliHowTo.SupportedVersion)
        {
    ...
    private static async Task WriteHeaderAsync(Stream stream, byte[] iv, CancellationToken cancellationToken)
    {
        ...
        stream.WriteByte(AesBrotliHowTo.SupportedVersion);
