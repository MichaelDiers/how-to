...
public class PackerStreamUnpackTests : PackerStreamCommonTests
{
    [Fact]
    public void CanRead()
    {
        ...
        Assert.True(packerStream.CanRead);
    ...
    public void CanWrite()
    {
        ...
        Assert.False(packerStream.CanWrite);
    ...
    [Fact]
    public void Read_Buffer_Offset_Count_IncompleteIv()
    {
        using var aes = Aes.Create();
        var memoryStream = new MemoryStream([2, 1]);
        var packerStream = new PackerStream(
            memoryStream,
        ...
        Assert.Equal(
            "Incomplete aes header: iv missing or incomplete.",
            exception.Message);
    ...
    [Fact]
    public void Read_Buffer_Offset_Count_MissingIv()
    {
        using var aes = Aes.Create();
    ...
    [Fact]
    public void Read_Buffer_Offset_Count_MissingIvLength()
    {
        using var aes = Aes.Create();
        var memoryStream = new MemoryStream();
        var packerStream = new PackerStream(
        ...
        Assert.Equal(
            "Missing aes header: Length of iv not specified.",
            exception.Message);
